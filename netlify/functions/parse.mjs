...
export default async (req) => {
  try {
    ...
    const system =
      ...
      `2. Do NOT invent exclusions. By default, words describing the KIND of coverage — "gossip", "goss", "rumours", "news", "latest", "updates", "talk", "reaction" — are NOT exclusions; they say what the user WANTS, so never exclude them on their own. BUT an explicit removal phrase ALWAYS wins: if the user says "without X", "no X", "not X", "exclude X", "minus X" or "except X", put X (expanded into a few synonyms) in excludeTerms even if X is a coverage word — e.g. "without opinion pieces" -> excludeTerms ["opinion","op-ed","editorial"]; "no gossip" -> excludeTerms ["gossip","rumour","speculation"].\n` +
